...
- Código: fundação executável, persistência base, valores de e-mail/senha, relógio, tokens seguros, retorno interno, contas, autorizações temporárias, sessões, limites de autenticação, casos de uso de identidade, Problem Details global, oito contratos REST, cookie seguro, guards de identidade, proteção de navegador, limites HTTP, e-mail SMTP com templates e entrega pós-commit, cliente de API e estado de autenticação concluídos.
- Próxima tarefa: `T-001-036` — implementar cadastro AUT-01.
